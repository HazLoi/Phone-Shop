...
class main
{
	...
	{
		global $fcm_secrect_key, $db_pos, $setup;
		$ch = curl_init();
		...
		// if( isset($post['notification']) && isset($post['notification']['is_sound']) && $post['notification']['is_sound'] == 1 ){
		// 	if( isset($setup['customize_push_sound']) && $setup['customize_push_sound'] == 1 ){
		// 		$post['notification']['sound'] = 'notification.wav';
		// 		$post['notification']['android_channel_id'] = 'vn.vinagroups.'.strtolower($db_pos);
		// 	}else{
		// 		$post['notification']['sound'] = 'default';
		// 	}
		// }
	...
	public function sendFCMV1($post, $project_id, $json)
	{
		require_once 'library/google_service_account/vendor/autoload.php';
		global $fcm_secrect_key, $db_pos, $setup;
		$ch = curl_init();
		...
		if (isset($post['notification'])) {
			$title = $post['notification']['title'] != '' ? $post['notification']['title'] : '';
			$body = $post['notification']['body'] != '' ? $post['notification']['body'] : '';
			$sound = $post['notification']['sound'] != '' ? $post['notification']['sound'] : '';
		}
		if (isset($post['data'])) {
			$force_refresh = $post['data']['force_refresh'] != '' ? $post['data']['force_refresh'] : '';
			$notify_id = $post['data']['notify_id'] != '' ? $post['data']['notify_id'] : '';
			$type = $post['data']['type'] != '' ? $post['data']['type'] : '';
			$value = $post['data']['value'] != '' ? $post['data']['value'] : '';
		}
		$message = [
			"message" => [
				"notification" => [
	...
	public function firebaseSMSOTPInfoToVerify($idToken)
	{
		global $setup;
		$ch = curl_init();

		$firebaseSecrect = isset($setup['firebase_secrect_key']) ? $setup['firebase_secrect_key'] : '';

		if ($firebaseSecrect != '') {
			$api = 'https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=' . $firebaseSecrect;
			$header = array(
	...
	public function getLongitudeLatitude($address)
	{
		...
		$YOUR_API_KEY = 'AIzaSyCEm46GOmLcr6cydfQzoJKD1ZOjIA1UyyU';
